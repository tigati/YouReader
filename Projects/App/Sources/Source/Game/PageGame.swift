...
extension GamePage {
    ...
    static func mapState(_ state: State, dispatch: @escaping Dispatch) -> GameVC.Props {
        ...
        let view = GameView.Props(pages: pages, didScroll: didScrollCommand)
    ...
    ) -> PageView.Props
    {
        ...
        let array = round.substrings.map(\.text)
    ...
    static func mapNote(dispatch: @escaping Dispatch, note: String) -> KeyView.Props {
        return .init(
            text: note,
